...
exports.getFacultyQuizzes = async (req, res) => {
  try {
    ...
    const quizzes = await Quiz.find({ createdBy: creatorId }).sort({
      createdAt: -1,
    });
    res.json(quizzes);
  } catch (err) {
...
exports.createQuiz = async (req, res) => {
  ...
  const { title, description, instructions, courseId, questions, isFinalExam } =
    req.body;
  ...
  try {
    ...
    for (const q of questions) {
      ...
      if (q.type === "radio" && correctCount !== 1) {
        return res
          .status(400)
          .json({
            message: `Single Choice Question "${q.questionText}" must have exactly ONE correct option.`,
          });
      }
      if (q.type === "checkbox" && correctCount < 1) {
        return res
          .status(400)
          .json({
            message: `Multiple Choice Question "${q.questionText}" must have AT LEAST ONE correct option.`,
          });
      }
    }
    ...
    try {
      const course =
        await Course.findById(courseId).select("name subject title");
      if (course) {
      ...
      console.error(
        "⚠️ Email setup failed inside Quiz Controller:",
        emailErr.message,
      );
    }
    // ------------------------------------------
    ...
    for (const q of questions) {
      ...
      if (q.type === "radio" && correctCount !== 1)
        return res
          .status(400)
          .json({
            message: `Radio Question "${q.questionText}" error: Must have exactly one correct option.`,
          });
      if (q.type === "checkbox" && correctCount < 1)
        return res
          .status(400)
          .json({
            message: `Checkbox Question "${q.questionText}" error: Must have at least one correct option.`,
          });
    }
...
// Get Quiz Results
// Get Quiz Results (All Students for a Quiz)
exports.getQuizResults = async (req, res) => {
  try {
    // The route is /quizzes/:quizId/results, so we get quizId from params
    const { quizId } = req.params;

    // Find all results linked to this quiz
    ...
    console.error("Error inside getQuizResults:", error);
    return res
      .status(500)
      .json({ message: "Server Error fetching quiz results." });
  }
};
...
exports.submitQuiz = async (req, res) => {
  try {
    ...
    for (let question of quiz.questions) {
      ...
      const studentSelectedOptions = userResponse
